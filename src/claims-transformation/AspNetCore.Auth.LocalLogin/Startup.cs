...
namespace AspNetCore.Auth.LocalLogin
{
    public class Startup
    {
        ...
        {
            services.AddMvc();
            var users = new Dictionary<string, string> { { "waqas", "password" } };
            services.AddSingleton<IUserService>(new DummyUserService(users));

            var profiles = new Dictionary<string, UserProfile>
            {
                {"waqas", new UserProfile("Mohammad","Waqas",new []{"User"}) }
            };
            services.AddSingleton<IProfileService>(new ProfileService(profiles));
